...
def register_router_tools(mcp: FastMCP) -> None:
    ...
    async def route_task(
        ...
        with tracked_activity():
            ...
            result = await run_blocking(route_task_api, task, mode=mode, token_budget=token_budget, top_k=top_k)
    ...
    async def execute_plan(
        ...
        with tracked_activity():
            ...
            result = await run_blocking(execute_plan_api, plan, arguments_by_tool=arguments_by_tool, confirmed=confirmed)
    ...
    async def search_context(query: str, project_root: str = "", top_k: int = 5, ctx: Context = None) -> str:
        ...
        with tracked_activity():
            ...
            result = await run_blocking(search_context_api, query, project_root=project_root, top_k=top_k)
    ...
    async def explain_plan(plan_json: str, ctx: Context = None) -> str:
        ...
        with tracked_activity():
            result = await run_blocking(explain_plan_api, _json_loads_object(plan_json))
    ...
    async def execute_selected_tool(
        ...
        with tracked_activity():
            ...
            result = await run_blocking(execute_selected_tool_api, tool_id, arguments, confirmed=confirmed)
    ...
    async def get_route_metrics(ctx: Context = None) -> str:
        ...
        with tracked_activity():
            result = await run_blocking(get_route_metrics_api)
    ...
    async def benchmark_router(iterations: int = 20, ctx: Context = None) -> str:
        ...
        with tracked_activity():
            result = await run_blocking(benchmark_route_task_api, iterations=iterations)
