...
from context_broker.storage import get_storage_config_info, list_saved_json, load_json_data, save_json_data
...
def register_storage_tools(mcp: FastMCP) -> None:
    ...
    async def save_search_results(
        ...
        try:
            ...
            data = {
                ...
                "files": [{"path": item["path"], "content": item["content"]} for item in result["results"]],
                "statistics": {
    ...
    async def list_saved_results(
        ...
        try:
            ...
            lines = [f"📁 Saved Results for: {project_name}", f"📦 Storage Mode: {STORAGE_MODE}", ""]
    ...
    async def get_storage_config(ctx: Context = None) -> str:
        ...
        lines = ["📦 Context Broker Storage Configuration", "", f"Current Mode: {config['mode']}", "", "Available Modes:"]
