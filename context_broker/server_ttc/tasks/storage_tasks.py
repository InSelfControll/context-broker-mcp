        ...
        with tracked_activity():
            ...
            lines.extend(
                [
                    ...
                    "",
                    "Cross-Chat Context:",
                    f"  Backend: {config['context']['backend']}",
                    f"  Honcho Workspace: {config['context']['honcho_workspace_id']}",
                    f"  Honcho Session Prefix: {config['context']['honcho_session_prefix']}",
                    f"  Honcho User Peer: {config['context']['honcho_user_peer_id']}",
                    f"  Honcho Assistant Peer: {config['context']['honcho_assistant_peer_id']}",
                    f"  Honcho Context Tokens: {config['context']['honcho_context_tokens']}",
                    f"  Honcho Limit To Session: {config['context']['honcho_limit_to_session']}",
                    f"  Redis URL: {config['context']['redis_url']}",
                    f"  Redis Key Prefix: {config['context']['redis_key_prefix']}",
                    f"  Chat Cache TTL (s): {config['context']['chat_cache_ttl_seconds']}",
                    "",
                    "User Identity:",
                    f"  Use Account Name: {config['identity']['use_account_name']}",
                    f"  Account Name Override: {config['identity']['account_name_override']}",
                    f"  Resolved User Peer ID: {config['identity']['resolved_user_peer_id']}",
                    "",
                    "Dashboard (web-only):",
                    f"  Host: {config['dashboard']['host']}",
